...
func NewWithGenesis(genesisBytes []byte, options ...Option) (App, string, map[string]json.RawMessage, *abci.RequestInitChain, *abci.ResponseInitChain, dbm.DB) {
	// homeDir := tempDir()
	homeDir, err := os.Getwd()
	if err != nil {
		panic(errors.Errorf("can't get current working directory: %s", err))
	}
	// dbPath := filepath.Join(homeDir, "data", "simapp-db.db")
	// if err := os.RemoveAll(dbPath); err != nil {
	// 	// panic(errors.Errorf("can't remove db directory: %s", err))
	// }
	// time.Sleep(1000 * time.Millisecond) // wait for the directory to be removed
	db, err := dbm.NewGoLevelDB("simapp-db", filepath.Join(homeDir, "data"), nil)
	if err != nil {
		panic(errors.Errorf("can't create db for simapp: %s", err))
	}
	...
	initChainResponse, err := coreApp.InitChain(initChainReq)
	if err != nil {
		panic(errors.Errorf("can't init chain: %s", err))
	}

	return App{*coreApp}, homeDir, appState, initChainReq, initChainResponse, db
}
