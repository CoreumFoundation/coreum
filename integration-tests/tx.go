...
func BroadcastTxWithSigner(ctx context.Context, chainCtx ChainContext, signerAddress sdk.AccAddress, msgs ...sdk.Msg) (*sdk.TxResponse, error) {
	...
	if txf.GasAdjustment() != 0 {
		gas = uint64(txf.GasAdjustment() * float64(gas))
	}
	txf = txf.WithGas(gas)
