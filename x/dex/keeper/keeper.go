...
func (k Keeper) GetOrdersWithSequence(
	...
	store := prefix.NewStore(runtime.KVStoreAdapter(s), types.OrderIDToSeqKeyPrefix)
...
func (k Keeper) removeOrderByRecord(
	...
	err = k.removeOrderData(ctx, record.OrderSeq)
	if err != nil {
		return err
	}
...
func (k Keeper) getPaginatedOrders(
	...
	s := k.storeService.OpenKVStore(ctx)
	store := prefix.NewStore(runtime.KVStoreAdapter(s), types.CreateOrderIDToSeqKeyPrefix(accNumber))
	orderBookIDToOrderBookData := make(map[uint32]types.OrderBookData)
	orders, pageRes, err := query.GenericFilteredPaginate(
		k.cdc,
		store,
...
func (k Keeper) removeOrderData(ctx sdk.Context, orderSeq uint64) error {
	return k.storeService.OpenKVStore(ctx).Delete(types.CreateOrderKey(orderSeq))
}
...
func (k Keeper) removeOrderIDToSeq(ctx sdk.Context, accNumber uint64, orderID string) error {
	return k.storeService.OpenKVStore(ctx).Delete(types.CreateOrderIDToSeqKey(accNumber, orderID))
}
