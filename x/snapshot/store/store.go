...
func (s SnapshotNameStore) Claim(index uint64, key []byte) error {
	s.fillSnapshot(key)
...
func (s SnapshotNameStore) fillSnapshot(key []byte) {
	...
	pendingIndex := pendingSnapshotStore.Index()
	for index := currentValueSnapshotStore.Index() + 1; index <= pendingIndex; index++ {
