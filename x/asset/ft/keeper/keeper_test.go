...
func TestKeeper_Burn_GovernanceDenom_Succeeds(t *testing.T) {
	...
	// Fund an account with governance denom
...
func TestKeeper_Burn_AssetFT_NoBurnFeature_Fails(t *testing.T) {
	requireT := require.New(t)
	...
	// Issue token without burn feature
	settings := types.IssueSettings{
		Issuer:        issuer,
		Symbol:        "NOBURN",
		Subunit:       "noburn",
		...
		Features:      []types.Feature{}, // No burn feature
	}
	...
	// Transfer to user
	err = bankKeeper.SendCoins(ctx, issuer, user, sdk.NewCoins(sdk.NewInt64Coin(denom, 1000)))
	requireT.NoError(err)

	// Try to burn - should fail
	err = ftKeeper.Burn(ctx, user, sdk.NewInt64Coin(denom, 100))
	requireT.Error(err)
	requireT.ErrorIs(err, types.ErrFeatureDisabled)
}
...
func TestKeeper_Burn_AssetFT_WithBurnFeature_Succeeds(t *testing.T) {
	...
	// Burn - should succeed
	err = ftKeeper.Burn(ctx, user, sdk.NewInt64Coin(denom, 250))
	requireT.NoError(err)

	supplyAfter := bankKeeper.GetSupply(ctx, denom)
	requireT.Equal(supplyBefore.Amount.SubRaw(250), supplyAfter.Amount)
}
