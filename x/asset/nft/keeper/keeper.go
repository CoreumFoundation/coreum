...
func (k Keeper) GetBurntNFTs(ctx sdk.Context, q *query.PageRequest) ([]types.BurntNFT, *query.PageResponse, error) {
	...
	pageRes, err := query.Paginate(prefix.NewStore(ctx.KVStore(k.storeKey), types.NFTBurningKeyPrefix),
		q, func(key, value []byte) error {
			if !bytes.Equal(value, asset.StoreTrue) {
				return sdkerrors.Wrapf(types.ErrInvalidInput, "value stored in burning store is not %x, value %x", asset.StoreTrue, value)
			}
...
func (k Keeper) GetFrozenNFTs(ctx sdk.Context, q *query.PageRequest) ([]types.FrozenNFT, *query.PageResponse, error) {
	...
	pageRes, err := query.Paginate(prefix.NewStore(ctx.KVStore(k.storeKey), types.NFTFreezingKeyPrefix),
		q, func(key, value []byte) error {
			if !bytes.Equal(value, asset.StoreTrue) {
				return sdkerrors.Wrapf(sdkerrors.ErrKeyNotFound, "value stored in freezing store is not %x, value %x", asset.StoreTrue, value)
			}
...
func (k Keeper) GetWhitelistedAccountsForNFT(ctx sdk.Context, classID, nftID string, q *query.PageRequest) ([]string, *query.PageResponse, error) {
	...
	pageRes, err := query.Paginate(prefix.NewStore(ctx.KVStore(k.storeKey), key),
		q, func(key, value []byte) error {
			if !bytes.Equal(value, asset.StoreTrue) {
				return sdkerrors.Wrapf(sdkerrors.ErrKeyNotFound, "value stored in whitelisting store is not %x, value %x", asset.StoreTrue, value)
			}
...
func (k Keeper) freezeOrUnfreeze(ctx sdk.Context, sender sdk.AccAddress, classID, nftID string, frozen bool) error {
	classDefinition, err := k.GetClassDefinition(ctx, classID)
	if err != nil {
		return err
	}
	...
	if err := k.SetFrozen(ctx, classID, nftID, frozen); err != nil {
		return err
	}
...
func (k Keeper) addToWhitelistOrRemoveFromWhitelist(ctx sdk.Context, classID, nftID string, sender, account sdk.AccAddress, whitelisting bool) error {
	classDefinition, err := k.GetClassDefinition(ctx, classID)
	if err != nil {
		return err
	}
	...
	if err := k.SetWhitelisting(ctx, classID, nftID, account, whitelisting); err != nil {
		return err
	}
