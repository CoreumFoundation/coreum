...
func (d *Direct) DeployBinary(ctx context.Context, app infra.Binary) (infra.DeploymentInfo, error) {
	...
	cmd := osexec.Command("/bin/sh", "-ce", fmt.Sprintf(`exec %s >> "%s/%s.log" 2>&1`, osexec.Command(binPath, app.ArgsFunc(d.BindIP(), d.config.AppDir+"/"+app.Name)...).String(), d.config.LogDir, app.Name))
