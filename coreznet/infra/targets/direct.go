...
func (d *Direct) DeployBinary(ctx context.Context, app infra.Binary) (infra.DeploymentInfo, error) {
	...
	cmd := osexec.Command("bash", "-ce", fmt.Sprintf(`exec %s >> "%s/%s.log" 2>&1`, osexec.Command(binPath, app.ArgsFunc(ipLocalhost, d.config.AppDir+"/"+app.Name, hostIPResolver{})...).String(), d.config.LogDir, app.Name))
